...
A supervised neural surrogate for predicting axisymmetric liquid-bridge equilibria governed by the Young–Laplace equation.
...
- **Pressure network:** Bond number → reference-pressure correction.
...
The exponential representation enforces positive radius. Boundary conditions and volume conservation are learned from the reference data rather than imposed exactly by the network architecture.
...
$$
\mathcal{L} = \frac{1}{2N_B}\sum_{b=1}^{N_B}\left\|\mathbf{U}_{\theta,b}-\mathbf{U}_{\mathrm{Newton},b}\right\|_2^2.
$$

Here, $N_B$ is the number of training Bond numbers. The loss is averaged over Bond numbers and summed over solution components.
...
### Train with a larger optimization budget
...
python train_yl_supervised.py \
    ...
    --steps 20000 \
    --lbfgs-steps 1000 \
    --output comparison_supervised_longer
```
...
## Results at unseen Bond numbers

The following results are from the larger-budget run above, using 100 finite elements, network width 32, two geometry hidden layers, one pressure hidden layer, and random seed 7.

All nine evaluation Bond numbers are excluded from training. Errors are measured against the Fortran Newton reference solutions.

| Metric | Mean across test cases | Maximum across test cases |
|---|---:|---:|
| Relative solution-vector error | 6.6694e-05 | 3.7900e-04 |
| Relative radius error | 7.8515e-05 | 4.6808e-04 |
| Relative axial-coordinate error | 5.3169e-05 | 2.9006e-04 |
| Absolute pressure error | 2.8004e-04 | 1.2028e-03 |

Relative errors use the L2 norm of the prediction error divided by the L2 norm of the reference quantity.

These values describe a single run; results may vary with the software environment.
...
The test cases measure interpolation within the training Bond-number range. They do not establish extrapolation performance.
...
The reported training tolerance applies to supervised data loss, not to the Young–Laplace residual. Boundary conditions and volume conservation are not guaranteed to hold exactly for neural predictions.
...
$$
z_{\mathrm{PINN}}=L-z_{\mathrm{Fortran}},\qquad K=P_0+GL.
$$
